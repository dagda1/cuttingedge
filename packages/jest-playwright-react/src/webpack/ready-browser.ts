...
if (urlParams.has('test')) {
  ...
  const component =
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ((window as any).__tests[testName] || {}).reactNode ||
    createElement('div', null, `no component found for test "${testName}"`);
  ...
  const createDetailsBlock = ([key, values]: any[]) => {
    ...
    if (!values.__reactNode) {
      ...
      a.onmouseenter = (e) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (e.target as any).style.setProperty('background-color', '#ddd');
      };
      a.onmouseleave = (e) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (e.target as any).style.removeProperty('background-color');
      };
      a.text = key;
