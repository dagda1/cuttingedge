...
const processFile = (sourceDir, filePath) =>
  readFile(filePath)
    ...
    .catch(err => {
      console.error(`Error in ${filePath} (${err.loc.line}:${err.loc.column}): ${err.message}`);
    });
...
const printKeys = keys => keys.forEach(key => console.error(key));
...
const report = result => {
  if (result) {
    ...
    console.log('Translations are fine');
    return true;
  }
};
