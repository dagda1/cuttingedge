...
const main = async () => {
  ...
  const [major, minor, patch] = versionBumpTypes.map(type =>
  ...
  if (choice.value === "custom") {
    ...
    if (!versionRegex.test(custom.value)) {
      console.log(
        "Version number format is incorrect. Please use the correct format"
      );
      return;
    }

    // check if the version number is smaller than earlier.
    let isValid = versionCompare(
      custom.value.replace(/[-,A-Z]/g, ""),
      currentVersion.replace(/[-,A-Z]/g, "")
    );
    if (!isValid) {
...
const bumpVersion = (version, type) => {
  return version
    .split(".")
    .map((factor, i) =>
      versionBumpTypes[i] === type
        ? Number(factor.match(/^[0-9]*/)[0]) + 1
        : factor
    )
    .join(".");
};
