...
module.exports = {
  webpack: function(config, { dev }) {
    ...
    config.module.rules = config.module.rules.filter(rule => rule.loader !== 'emit-file-loader');

    // Remove chunking plugins that cause problems
    config.plugins = config.plugins.filter(plugin => !/Chunk/.test(plugin.constructor.name));
